...
管理员的「资源管理」会列出扫码、备用、账号调试和任务浏览器，并通过 Worker 心跳和 Redis 截图请求显示正在运行的窗口。启用这一页需要同时更新 API、Web 和三个浏览器 Worker；此功能不需要新的数据库迁移。手动关闭任务浏览器会移除该账号的任务分配，若它是最后一个执行账号，任务会停止。

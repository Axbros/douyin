...
const taskForm = ref({ room_id: '', script_ids: [] as number[], min_interval_seconds: 20, max_interval_seconds: 50, account_source: 'platform', account_ids: [] as number[] })
...
const customerReadySteps = computed(() => [
  ...
  { label: '创建直播任务', done: activeCustomerTaskCount.value > 0, detail: activeCustomerTaskCount.value ? '当前已有活动任务' : '填写直播间 ID 后确认创建', target: 'tasks' },
])
...
function openCreateTask() {
  ...
  taskForm.value = { room_id: '', script_ids: [], min_interval_seconds: minInterval, max_interval_seconds: maxInterval, account_source: 'platform', account_ids: [] }
...
async function createCustomerTask() {
  ...
  const confirmed = await ElMessageBox.confirm(`将使用${modeName}（${accountCount} 个）创建任务，本次费用 ¥${(selectedTaskPrice.value / 100).toFixed(2)}。确认继续吗？`, '确认创建直播任务', { confirmButtonText: '确认创建', cancelButtonText: '返回修改', type: 'warning' }).catch(() => false)
  if (!confirmed) return
...
<template>
        ...
        <template v-else-if="active === 'accounts'">
          ...
          <el-card shadow="never">
            ...
            <el-table v-loading="loading" :data="paginatedAccounts" border stripe style="width: 100%" empty-text="暂无数据">
              <el-table-column prop="id" label="ID" width="72" /><el-table-column prop="display_name" label="账号名称" min-width="150" /><el-table-column label="归属" width="120"><template #default="{ row }"><el-tag :type="row.ownership_type === 'platform' ? 'primary' : 'success'">{{ row.ownership_type === 'platform' ? '平台账号' : '客户自有' }}</el-tag></template></el-table-column>
  ...
  <div v-else class="customer-app" v-loading="customerLoading">
    ...
    <el-dialog v-model="createTaskVisible" title="创建直播任务" width="min(92vw, 620px)" align-center><el-form label-position="top"><el-form-item label="账号模式"><el-radio-group v-model="taskForm.account_source" class="mode-options" @change="taskForm.account_ids = []"><el-radio-button value="customer">我的账号 · ¥{{ (platformSettings.customer_account_task_price_cents / 100).toFixed(2) }}</el-radio-button><el-radio-button value="platform">平台账号 · ¥{{ (platformSettings.platform_account_task_price_cents / 100).toFixed(2) }}</el-radio-button></el-radio-group></el-form-item><el-alert v-if="taskForm.account_source === 'platform'" :title="`平台将随机分配 ${platformSettings.default_target_account_count} 个闲置账号`" type="info" :closable="false" /><template v-else><el-form-item label="选择我的可用账号"><el-checkbox-group v-model="taskForm.account_ids" class="script-options"><el-checkbox v-for="account in availableCustomerAccounts" :key="account.id" :value="account.id">{{ account.display_name }}</el-checkbox></el-checkbox-group></el-form-item><el-alert v-if="!availableCustomerAccounts.length" title="没有可用的自有账号，请先到“我的抖音号”添加并完成登录" type="warning" :closable="false" /></template><el-form-item label="直播间 ID"><el-input v-model="taskForm.room_id" placeholder="请输入直播间 ID" /></el-form-item><el-form-item :label="`选择审核通过的话术（最多 ${platformSettings.max_scripts_per_task} 条）`"><el-checkbox-group v-model="taskForm.script_ids" class="script-options"><el-checkbox v-for="script in approvedCustomerScripts" :key="script.id" :value="script.id"><span>{{ script.content }}</span></el-checkbox></el-checkbox-group></el-form-item><div class="interval-row"><el-form-item label="最小间隔（秒）"><el-input-number v-model="taskForm.min_interval_seconds" :min="platformSettings.comment_min_interval_seconds" :max="platformSettings.comment_max_interval_seconds" /></el-form-item><el-form-item label="最大间隔（秒）"><el-input-number v-model="taskForm.max_interval_seconds" :min="platformSettings.comment_min_interval_seconds" :max="platformSettings.comment_max_interval_seconds" /></el-form-item></div><div class="price-summary"><span>本次任务费用</span><strong>¥{{ (selectedTaskPrice / 100).toFixed(2) }}</strong></div></el-form><template #footer><el-button @click="createTaskVisible = false">取消</el-button><el-button type="primary" :loading="createTaskLoading" @click="createCustomerTask">创建任务</el-button></template></el-dialog>

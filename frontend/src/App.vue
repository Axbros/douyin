...
const clickingLogin = ref(false)
const previousQrPayload = ref('')
const loginStatus = ref('waiting')
const qrTimer = ref<number | null>(null)
const loginSessionId = ref<number | null>(null)
const loginBrowserId = ref('')
const loginBrowserOrigin = ref('')
...
const loginPasswordVerifySubmitting = ref(false)
const browserPreviewVisible = ref(false)
const browserPreviewLoading = ref(false)
const browserPreviewUrl = ref('')
const browserPreviewTitle = ref('当前浏览器画面')
const browserPreviewPath = ref('')
const detectingLoginQr = ref(false)
...
const accountCommentLogs = ref<any[]>([])
...
async function qrLogin(id: number, scope: 'admin' | 'customer' = 'admin') {
  ...
  loginBrowserId.value = ''; loginBrowserOrigin.value = ''
  qr.value = ''; loginSessionId.value = null; loginStatus.value = 'waiting'; qrLoading.value = true; qrRefreshing.value = false; clickingLogin.value = false; previousQrPayload.value = ''; detectingLoginQr.value = false; verificationCode.value = ''; verificationHint.value = ''; verificationOptions.value = []; verificationMethodId.value = ''; selectedVerificationMethod.value = null; loginPassword.value = ''; loginPasswordVerifySubmitting.value = false; qrDialogVisible.value = true
  ...
  qrTimer.value = window.setInterval(async () => {
    ...
    loginBrowserId.value = session.browser_id || loginBrowserId.value
    loginBrowserOrigin.value = session.browser_origin || loginBrowserOrigin.value
    if (session.qr_payload && !qrRefreshing.value && session.qr_payload !== previousQrPayload.value) {
      const nextQr = `data:image/png;base64,${session.qr_payload}`
      if (qr.value !== nextQr) { qr.value = nextQr; qrLoading.value = false; notify('登录二维码已获取', 'success') }
    }
    ...
    if (['password_required', 'password_filling', 'password_ready', 'password_clicking', 'password_verifying'].includes(session.status)) {
...
async function detectLoginQr() {
  const id = loginSessionId.value
  if (!id || detectingLoginQr.value) return
  detectingLoginQr.value = true
  try {
    const response = await fetch(`/api/${loginApiScope.value}/douyin-login-sessions/${id}/detect-qr`, { method: 'POST', headers: authHeaders() })
    const data = await response.json()
    if (loginSessionId.value !== id) return
    if (!response.ok) { notify(data.detail || '检测登录二维码失败', 'error'); return }
    if (data.found && data.qr_payload) {
      qr.value = `data:image/png;base64,${data.qr_payload}`
      qrLoading.value = false
      notify('登录二维码已获取', 'success')
    } else if (!qr.value) notify(data.message || '当前页面尚未出现登录二维码，请稍后重试', 'warning')
  } catch { if (loginSessionId.value === id) notify('检测登录二维码失败，请稍后重试', 'error') }
  finally { detectingLoginQr.value = false }
}
async function clickLoginInBrowser() {
  const id = loginSessionId.value
  if (!id || clickingLogin.value || loginStatus.value !== 'waiting') return
  clickingLogin.value = true
  try {
    const response = await fetch(`/api/${loginApiScope.value}/douyin-login-sessions/${id}/click-login`, { method: 'POST', headers: authHeaders() })
    const data = await response.json()
    if (loginSessionId.value !== id) return
    notify(data.message || data.detail || (response.ok ? '已向浏览器发送点击指令' : '点击登录失败'), !response.ok ? 'error' : data.clicked ? 'success' : 'warning')
  } catch { if (loginSessionId.value === id) notify('点击浏览器登录按钮失败，请稍后重试', 'error') }
  finally { clickingLogin.value = false }
}
async function refreshLoginQr() {
  const id = loginSessionId.value
  if (!id || !qr.value || qrRefreshing.value || loginStatus.value !== 'waiting') return
  previousQrPayload.value = qr.value.split(',', 2)[1] || ''
  qrRefreshing.value = true
  qrLoading.value = true
  qr.value = ''
  try {
    const response = await fetch(`/api/${loginApiScope.value}/douyin-login-sessions/${id}/refresh-qr`, { method: 'POST', headers: authHeaders() })
    const data = await response.json()
    if (loginSessionId.value !== id) return
    if (!response.ok) { notify(data.detail || '重新获取二维码失败', 'error'); return }
    if (data.found && data.qr_payload) {
      qr.value = `data:image/png;base64,${data.qr_payload}`
      notify('已获取新的登录二维码', 'success')
    } else notify(data.message || '刷新后暂未出现二维码，请稍后手动获取', 'warning')
  } catch { if (loginSessionId.value === id) notify('重新获取二维码失败，请稍后重试', 'error') }
  finally { qrRefreshing.value = false; qrLoading.value = false }
}
...
async function submitLoginPassword() {
  ...
  try {
    ...
    loginStatus.value = 'password_filling'; loginPassword.value = ''; verificationHint.value = ''; notify('正在将登录密码填入浏览器', 'success')
  } finally { loginPasswordSubmitting.value = false }
}
async function verifyLoginPassword() {
  const id = loginSessionId.value
  if (!id || loginStatus.value !== 'password_ready') { notify('请等待密码填写完成', 'warning'); return }
  loginPasswordVerifySubmitting.value = true
  try {
    const response = await fetch(`/api/${loginApiScope.value}/douyin-login-sessions/${id}/verify-login-password`, {
      method: 'POST', headers: authHeaders(),
    })
    const data = await response.json()
    if (!response.ok) { notify(data.detail || '执行密码验证失败', 'error'); return }
    loginStatus.value = 'password_clicking'; verificationHint.value = ''; notify('正在点击浏览器中的验证按钮', 'success')
  } finally { loginPasswordVerifySubmitting.value = false }
}
function clearBrowserPreview() {
  if (browserPreviewUrl.value) URL.revokeObjectURL(browserPreviewUrl.value)
  browserPreviewUrl.value = ''
}
async function showBrowserPreview(path: string, title: string) {
  clearBrowserPreview()
  browserPreviewPath.value = path
  browserPreviewTitle.value = title
  browserPreviewVisible.value = true
  browserPreviewLoading.value = true
  try {
    const response = await fetch(path, { headers: authHeaders() })
    if (!response.ok) {
      let message = '获取浏览器画面失败'
      try { message = (await response.json()).detail || message } catch {}
      notify(message, 'error')
      return
    }
    browserPreviewUrl.value = URL.createObjectURL(await response.blob())
  } catch {
    notify('获取浏览器画面失败，请确认浏览器仍在运行', 'error')
  } finally { browserPreviewLoading.value = false }
}
function refreshBrowserPreview() {
  if (browserPreviewPath.value) showBrowserPreview(browserPreviewPath.value, browserPreviewTitle.value)
}
function showLoginBrowserPreview() {
  if (!loginSessionId.value) { notify('登录浏览器尚未启动', 'warning'); return }
  showBrowserPreview(`/api/admin/douyin-login-sessions/${loginSessionId.value}/screenshot`, '扫码登录浏览器画面')
}
...
function openResourceAddress(row: any) {
  ...
  resourceAddressUrl.value = row.url || 'https://live.douyin.com/'
...
async function showAccountLogs(row: any) {
  ...
  try {
    const [eventResponse, commentResponse] = await Promise.all([
      fetch(`/api/admin/douyin-accounts/${row.id}/logs`, { headers: authHeaders() }),
      fetch(`/api/admin/douyin-accounts/${row.id}/comment-logs`, { headers: authHeaders() }),
    ])
    if (!eventResponse.ok || !commentResponse.ok) { notify('读取账号日志失败', 'error'); return }
    accountLogs.value = await eventResponse.json()
    accountCommentLogs.value = await commentResponse.json()
  ...
  <div v-if="!token" class="login-page">
    ...
    <main class="login-content">
      <section class="login-intro">
        <el-tag effect="dark" round>抖无忧 · 直播运营平台</el-tag>
        <h1>让直播运营<br>更清晰、更省心</h1>
        <p>账号管理、话术审核、任务执行与评论记录集中在一个平台完成。</p>
        <div class="login-features"><div><strong>独立</strong><span>账号环境</span></div><div><strong>实时</strong><span>任务状态</span></div><div><strong>清晰</strong><span>评论记录</span></div></div>
      </section>
      <section class="login-card-wrap">
        <div class="login-card-glow" />
        <el-card class="login-card" shadow="never">
          <div class="login-card-title">欢迎登录抖无忧</div><p>请输入账号和密码进入系统</p>
  ...
  <el-container v-else-if="role === 'admin'" class="admin-shell">
        ...
        <template v-else-if="active === 'resources'">
          ...
          <el-card shadow="never">
            ...
            <el-table v-loading="browserResourcesLoading" :data="filteredBrowserResources" border stripe empty-text="暂无运行中的浏览器">
              <el-table-column type="index" label="序号" width="70" />
              <el-table-column label="类型" width="150"><template #default="{ row }"><el-tag :type="row.kind === 'task' ? 'warning' : row.kind === 'warm' ? 'info' : 'primary'">{{ resourceKindText(row.kind) }}</el-tag></template></el-table-column><el-table-column label="浏览器 ID" width="170"><template #default="{ row }"><el-tooltip v-if="row.browser_id" :content="row.browser_id"><code>{{ row.browser_id.slice(0, 12) }}</code></el-tooltip><span v-else>—</span></template></el-table-column>
    ...
    <el-dialog v-model="accountLogVisible" :title="`${accountLogTarget?.display_name || '抖音账号'} · 账号日志`" width="min(94vw, 1080px)" align-center>
      <el-tabs v-loading="accountLogLoading">
        <el-tab-pane :label="`评论日志（${accountCommentLogs.length}）`">
          <el-table :data="accountCommentLogs" border stripe max-height="520" empty-text="暂无评论日志"><el-table-column type="index" label="序号" width="70" /><el-table-column label="时间" width="180"><template #default="{ row }">{{ formatDate(row.sent_at || row.created_at) }}</template></el-table-column><el-table-column label="直播间" min-width="260" show-overflow-tooltip><template #default="{ row }"><el-link :href="row.live_url" target="_blank" type="primary">{{ row.live_url }}</el-link></template></el-table-column><el-table-column prop="content" label="评论内容" min-width="220" show-overflow-tooltip /><el-table-column label="结果" width="90"><template #default="{ row }"><el-tag :type="row.result === 'sent' ? 'success' : 'danger'">{{ row.result === 'sent' ? '成功' : '失败' }}</el-tag></template></el-table-column><el-table-column label="失败原因" min-width="190" show-overflow-tooltip><template #default="{ row }">{{ row.result === 'sent' ? '—' : (row.failure_reason || row.failure_code || '评论发送失败') }}</template></el-table-column></el-table>
        </el-tab-pane>
        <el-tab-pane :label="`运行日志（${accountLogs.length}）`">
          <el-table :data="accountLogs" border stripe max-height="520" empty-text="暂无运行日志"><el-table-column type="index" label="序号" width="70" /><el-table-column label="时间" width="180"><template #default="{ row }">{{ formatDate(row.created_at) }}</template></el-table-column><el-table-column label="事件" width="170"><template #default="{ row }">{{ accountEventText(row.event_type) }}</template></el-table-column><el-table-column label="详细信息" min-width="420" show-overflow-tooltip><template #default="{ row }">{{ formatLogDetail(row.detail) }}</template></el-table-column></el-table>
        </el-tab-pane>
      </el-tabs>
    </el-dialog>
    ...
    <el-dialog v-model="browserPreviewVisible" :title="browserPreviewTitle" width="min(94vw, 1100px)" align-center append-to-body @closed="clearBrowserPreview">
      <div v-loading="browserPreviewLoading" class="browser-preview-stage">
        ...
        <el-empty v-else-if="!browserPreviewLoading" description="暂未获取到浏览器画面" />
      </div>
...
.login-page{position:relative;min-height:100vh;overflow:hidden;color:#24324a;background:#f3f6fb}.login-bg{position:absolute;inset:0;background:radial-gradient(760px 520px at 8% 15%,rgba(76,111,255,.15),transparent 64%),radial-gradient(700px 520px at 92% 82%,rgba(83,196,174,.13),transparent 66%),linear-gradient(145deg,#f8faff 0%,#eef3fb 100%)}.login-grid{position:absolute;inset:0;opacity:.55;background-image:linear-gradient(rgba(89,112,164,.055) 1px,transparent 1px),linear-gradient(90deg,rgba(89,112,164,.055) 1px,transparent 1px);background-size:32px 32px;mask-image:linear-gradient(to bottom,#000,transparent 88%)}.login-orb{position:absolute;width:440px;height:440px;border-radius:50%;filter:blur(90px);opacity:.28}.orb-one{left:-240px;top:160px;background:#5a7cff}.orb-two{right:-210px;bottom:-180px;background:#55c6ae}.login-topbar{position:relative;z-index:1;height:82px;display:flex;align-items:center;justify-content:space-between;padding:0 clamp(24px,4vw,58px);border-bottom:1px solid rgba(117,136,178,.14);background:rgba(255,255,255,.64);backdrop-filter:blur(14px)}.brand-on-dark{color:#24324a}.brand-on-dark small{display:block;margin-top:2px;color:#8791a4;font-size:11px}.security-badges{display:flex;gap:9px}.security-badges span{padding:7px 11px;color:#647189;font-size:11px;border:1px solid #dfe5ef;border-radius:999px;background:rgba(255,255,255,.82)}.login-content{position:relative;z-index:1;width:min(1080px,calc(100% - 48px));min-height:calc(100vh - 82px);display:grid;grid-template-columns:1.08fr .82fr;align-items:center;gap:88px;margin:0 auto;padding:54px 0 88px}.login-intro .el-tag{padding:0 13px;border-color:#4b6de7;background:#4b6de7}.login-intro h1{margin:22px 0 18px;color:#1e2b43;font-size:clamp(40px,5vw,61px);font-weight:760;line-height:1.12;letter-spacing:-1.8px}.login-intro>p{max-width:540px;margin:0;color:#69758a;font-size:16px;line-height:1.85}.login-features{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:32px}.login-features div{min-width:0;padding:16px 17px;border:1px solid #e1e7f0;border-radius:14px;background:rgba(255,255,255,.78);box-shadow:0 8px 24px rgba(57,76,116,.055)}.login-features strong,.login-features span{display:block}.login-features strong{color:#32415d;font-size:17px}.login-features span{margin-top:5px;color:#8a95a7;font-size:12px}.login-card-wrap{position:relative}.login-card-glow{position:absolute;inset:16px -14px -18px;border-radius:28px;background:rgba(72,105,222,.16);filter:blur(24px)}.login-card{position:relative;border:1px solid #e0e6f0;border-radius:20px;background:#fff;box-shadow:0 22px 54px rgba(45,62,98,.12)}.login-card :deep(.el-card__body){padding:36px}.login-card-title{color:#202d44;font-size:23px;font-weight:700}.login-card p{margin:8px 0 28px;color:#8a94a5;font-size:13px}.login-card :deep(.el-form-item__label){color:#4f5d73;font-weight:600}.login-card :deep(.el-input__wrapper){min-height:46px;background:#f8fafc;box-shadow:0 0 0 1px #dfe5ee inset}.login-card :deep(.el-input__wrapper:hover){box-shadow:0 0 0 1px #aab9d6 inset}.login-card :deep(.el-input__wrapper.is-focus){background:#fff;box-shadow:0 0 0 1px var(--el-color-primary) inset}.login-card :deep(.el-input__inner){color:#27344b}.login-card :deep(.el-input__prefix),.login-card :deep(.el-input__suffix){color:#8a96aa}.login-card .el-button{width:100%;height:46px;margin-top:6px;border-radius:9px;font-weight:600;box-shadow:0 8px 18px rgba(64,113,238,.2)}

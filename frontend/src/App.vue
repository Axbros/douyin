...
import { Avatar, ChatDotRound, CirclePlus, Connection, Document, HomeFilled, Key, Lock, Monitor, Operation, Refresh, Search, Setting, Tickets, User, UserFilled, Warning } from '@element-plus/icons-vue'
...
const customerForm = ref({ id: 0, login: '', display_name: '', password: '' })
...
const taskForm = ref({ room_id: '', script_ids: [] as number[], min_interval_seconds: 20, max_interval_seconds: 50 })
...
const filteredTasks = computed(() => tasks.value.filter(task =>
  (!taskKeyword.value || String(task.id).includes(taskKeyword.value) || (task.room_id || '').includes(taskKeyword.value)) &&
...
async function qrLogin(id: number) {
  ...
  const response = await fetch(`/api/admin/douyin-accounts/${id}/login-session`, { method: 'POST', headers: authHeaders() })
  const data = await response.json()
  ...
  qrTimer.value = window.setInterval(async () => {
    const statusResponse = await fetch(`/api/admin/douyin-login-sessions/${data.id}`, { headers: authHeaders() })
    ...
    if (session.status === 'success') { loginStatus.value = 'success'; qrLoading.value = false; stopQrTimer(); notify('抖音账号登录成功', 'success'); await load() }
...
async function submitVerificationMethod() {
  ...
  try {
    const response = await fetch(`/api/admin/douyin-login-sessions/${id}/verification-method`, {
...
async function submitLoginPassword() {
  ...
  try {
    const response = await fetch(`/api/admin/douyin-login-sessions/${id}/login-password`, {
...
async function submitVerificationCode() {
  ...
  try {
    const response = await fetch(`/api/admin/douyin-login-sessions/${id}/verification-code`, {
...
async function closeQr(done?: () => void) {
  ...
  if (id) {
    const response = await fetch(`/api/admin/douyin-login-sessions/${id}/close`, { method: 'POST', headers: authHeaders() })
    if (response.ok) { notify('扫码窗口和浏览器已关闭', 'success'); await load() } else notify('关闭浏览器失败', 'error')
  }
...
async function wakeBrowser(id: number) {
  ...
  try {
    ...
    if (response.ok) { notify('调试浏览器正在启动，将自动恢复该账号登录状态', 'success'); window.setTimeout(load, 1200) }
...
async function closeBrowser(id: number) {
  ...
  if (response.ok) { notify('浏览器关闭指令已发送', 'success'); window.setTimeout(load, 800) } else notify('关闭失败', 'error')
}
...
function openCreateCustomer() {
  ...
  customerForm.value = { id: 0, login: '', display_name: '', password: '' }
  customerDialogVisible.value = true
}
function openEditCustomer(row: any) {
  customerDialogMode.value = 'edit'
  customerForm.value = { id: row.id, login: row.login, display_name: row.display_name, password: '' }
  customerDialogVisible.value = true
}
async function submitCustomer() {
  ...
  try {
    ...
    const response = await fetch(creating ? '/api/admin/customers' : `/api/admin/customers/${form.id}`, {
      ...
      body: JSON.stringify(creating ? { login: form.login.trim(), display_name: form.display_name.trim(), password: form.password } : { login: form.login.trim(), display_name: form.display_name.trim() }),
    })
    if (!response.ok) { notify((await response.json()).detail || '保存失败', 'error'); return }
...
async function rejectScript() {
  ...
  const response = await fetch(`/api/admin/scripts/${rejectForm.value.id}/reject?reason=${encodeURIComponent(rejectForm.value.reason.trim())}`, { method: 'POST', headers: authHeaders() })
  if (!response.ok) { notify((await response.json()).detail || '审核失败', 'error'); return }
  rejectVisible.value = false; notify('话术已拒绝', 'success'); await load()
}
async function loadCustomer() {
  if (role.value !== 'customer') return
  customerLoading.value = true
  try {
    const [meResponse, scriptResponse, taskResponse, settingResponse] = await Promise.all([
    ...
    if ([meResponse, scriptResponse, taskResponse, settingResponse].some(response => response.status === 401)) { logout(); return }
...
async function importScripts() {
  ...
  try {
    ...
    importVisible.value = false; importText.value = ''; notify(`成功导入 ${contents.length} 条话术`, 'success'); await loadCustomer()
...
function openCreateTask() {
  ...
  taskForm.value = { room_id: '', script_ids: [], min_interval_seconds: minInterval, max_interval_seconds: maxInterval }
...
async function createCustomerTask() {
  ...
  try {
    const response = await fetch('/api/customer/tasks', { method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, room_id: form.room_id.trim() }) })
    if (!response.ok) { notify((await response.json()).detail || '创建任务失败', 'error'); return }
    createTaskVisible.value = false; notify('直播任务创建成功，系统已分配账号', 'success'); customerActive.value = 'tasks'; await loadCustomer()
...
<template>
  <div v-if="!token" class="login-page">
    ...
    <header class="login-topbar">
      <div class="brand brand-on-dark"><span class="brand-logo"><Connection /></span><div><strong>Douyin Control</strong><small>直播运营管理平台</small></div></div>
  ...
  <el-container v-else-if="role === 'admin'" class="admin-shell">
    <el-header class="app-header">
      <div class="brand"><span class="brand-logo"><Connection /></span><strong>Douyin Control</strong><el-tag type="danger" effect="dark" size="small">ADMIN</el-tag></div>
      ...
      <el-main class="app-main">
        ...
        <template v-else-if="active === 'settings'">
          ...
          <el-card shadow="never" class="settings-card">
            <div class="settings-section"><div class="settings-title"><strong>任务分配</strong><span>控制客户任务数量和系统分配的抖音账号</span></div><div class="settings-grid"><el-form-item label="每个任务分配账号数"><el-input-number v-model="platformSettings.default_target_account_count" :min="1" :max="100" /></el-form-item><el-form-item label="单客户活动任务上限"><el-input-number v-model="platformSettings.max_active_tasks_per_customer" :min="1" :max="20" /></el-form-item><el-form-item label="单任务最多话术数"><el-input-number v-model="platformSettings.max_scripts_per_task" :min="1" :max="500" /></el-form-item><el-form-item label="单次话术导入上限"><el-input-number v-model="platformSettings.script_bulk_import_limit" :min="1" :max="1000" /></el-form-item></div></div>
        ...
        <template v-else-if="active === 'accounts'">
          ...
          <el-card shadow="never">
            ...
            <el-table v-loading="loading" :data="paginatedAccounts" border stripe style="width: 100%" empty-text="暂无数据">
              <el-table-column prop="id" label="ID" width="72" /><el-table-column prop="display_name" label="账号名称" min-width="150" />
              ...
              <el-table-column label="当前任务" width="110"><template #default="{ row }">{{ row.current_task_id || '—' }}</template></el-table-column>
        ...
        <template v-else-if="active === 'tasks'">
          <el-card shadow="never" style="margin-bottom: 12px">
            ...
            <el-form :model="{ taskKeyword, taskStatus, taskCustomerId }" inline label-position="right" class="queryForm">
              <el-form-item label="任务"><el-input v-model="taskKeyword" clearable placeholder="请输入任务 ID / 直播间 ID" :prefix-icon="Search" /></el-form-item>
        ...
        <template v-else-if="active === 'customers'">
          ...
          <el-card shadow="never">
            ...
            <el-table v-loading="loading" :data="paginatedCustomers" border stripe style="width: 100%" empty-text="暂无数据">
              <el-table-column prop="id" label="ID" width="80" fixed="left" /><el-table-column prop="login" label="登录名" min-width="150" /><el-table-column prop="display_name" label="客户名称" min-width="160" />
        ...
        <template v-else-if="active === 'scripts'">
          ...
          <el-card shadow="never">
            <div class="table-toolbar"><div class="table-title">话术审核列表</div><div class="record-total">共 {{ filteredScripts.length }} 条记录</div></div>
            <el-table v-loading="loading" :data="paginatedScripts" border stripe style="width: 100%" empty-text="暂无数据">
    ...
    <el-dialog v-model="customerDialogVisible" :title="customerDialogMode === 'create' ? '新增客户' : '编辑客户'" width="520px" align-center>
      <el-form label-position="left" label-width="100px"><el-form-item label="登录名"><el-input v-model="customerForm.login" autocomplete="off" maxlength="190" style="width: 280px" /></el-form-item><el-form-item label="客户名称"><el-input v-model="customerForm.display_name" autocomplete="off" maxlength="100" style="width: 280px" /></el-form-item><el-form-item v-if="customerDialogMode === 'create'" label="初始密码"><el-input v-model="customerForm.password" autocomplete="new-password" type="password" show-password maxlength="128" style="width: 280px" /></el-form-item></el-form>
    ...
    <el-dialog v-model="rejectVisible" title="拒绝话术" width="520px" align-center>
      <el-form label-position="left" label-width="90px"><el-form-item label="话术标题"><span>{{ rejectForm.title }}</span></el-form-item><el-form-item label="拒绝原因"><el-input v-model="rejectForm.reason" type="textarea" :rows="4" maxlength="500" show-word-limit style="width: 340px" /></el-form-item></el-form>
      <template #footer><el-button @click="rejectVisible = false">取消</el-button><el-button type="danger" @click="rejectScript">确认拒绝</el-button></template>
    </el-dialog>
  ...
  <div v-else class="customer-app" v-loading="customerLoading">
    <header class="customer-header"><div><strong>直播助手</strong><span>{{ customerMe?.display_name || displayName }}</span></div><div class="expiry-chip"><small>服务有效期</small><b>{{ customerRemainingText }}</b></div></header>
    <main class="customer-main">
      <template v-if="customerActive === 'home'">
        ...
        <section class="mobile-card"><div class="mobile-card-title"><strong>当前任务</strong><el-button type="primary" size="small" @click="openCreateTask">新建任务</el-button></div><div v-if="activeCustomerTask" class="active-task"><div><span>直播间</span><b>{{ activeCustomerTask.room_id }}</b></div><el-tag :type="taskStatusMeta(activeCustomerTask.status)[1]">{{ taskStatusMeta(activeCustomerTask.status)[0] }}</el-tag></div><el-empty v-else description="暂无活动任务" :image-size="72" /></section>
      </template>

      <template v-else-if="customerActive === 'scripts'">
        <div class="mobile-page-title"><div><h2>我的话术</h2><p>导入后提交管理员审核</p></div><el-button type="primary" @click="importVisible = true">批量导入</el-button></div>
        <section v-for="script in customerScripts" :key="script.id" class="mobile-card script-card"><div class="mobile-card-title"><strong>{{ script.title }}</strong><el-tag :type="scriptStatusMeta(script.status)[1]">{{ scriptStatusMeta(script.status)[0] }}</el-tag></div><p>{{ script.content }}</p><div class="script-footer"><span v-if="script.review_reason">原因：{{ script.review_reason }}</span><span v-else>权重 {{ script.weight }}</span><el-button v-if="['draft', 'rejected'].includes(script.status)" link type="primary" @click="submitCustomerScript(script)">提交审核</el-button></div></section>
    ...
    <el-dialog v-model="importVisible" title="批量导入话术" width="min(92vw, 520px)" align-center><p class="dialog-help">每行填写一条话术，单次最多导入 {{ platformSettings.script_bulk_import_limit }} 条。</p><el-input v-model="importText" type="textarea" :rows="10" placeholder="欢迎来到直播间&#10;喜欢的朋友可以关注一下&#10;有问题可以在评论区留言" /><template #footer><el-button @click="importVisible = false">取消</el-button><el-button type="primary" :loading="importLoading" @click="importScripts">确认导入</el-button></template></el-dialog>
    <el-dialog v-model="createTaskVisible" title="创建直播任务" width="min(92vw, 560px)" align-center><el-form label-position="top"><el-form-item label="直播间 ID"><el-input v-model="taskForm.room_id" placeholder="请输入直播间 ID" /></el-form-item><el-form-item :label="`选择审核通过的话术（最多 ${platformSettings.max_scripts_per_task} 条）`"><el-checkbox-group v-model="taskForm.script_ids" class="script-options"><el-checkbox v-for="script in approvedCustomerScripts" :key="script.id" :value="script.id"><span>{{ script.content }}</span></el-checkbox></el-checkbox-group></el-form-item><div class="interval-row"><el-form-item label="最小间隔（秒）"><el-input-number v-model="taskForm.min_interval_seconds" :min="platformSettings.comment_min_interval_seconds" :max="platformSettings.comment_max_interval_seconds" /></el-form-item><el-form-item label="最大间隔（秒）"><el-input-number v-model="taskForm.max_interval_seconds" :min="platformSettings.comment_min_interval_seconds" :max="platformSettings.comment_max_interval_seconds" /></el-form-item></div><el-alert :title="`系统将自动随机分配 ${platformSettings.default_target_account_count} 个闲置抖音账号`" type="info" :closable="false" /></el-form><template #footer><el-button @click="createTaskVisible = false">取消</el-button><el-button type="primary" :loading="createTaskLoading" @click="createCustomerTask">创建任务</el-button></template></el-dialog>

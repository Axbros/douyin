...
const taskForm = ref({ room_id: '', script_ids: [] as number[], min_interval_seconds: 20, max_interval_seconds: 50, account_source: 'platform', account_ids: [] as number[], script_order_mode: 'random' })
...
function openCreateTask() {
  ...
  taskForm.value = { room_id: '', script_ids: [], min_interval_seconds: minInterval, max_interval_seconds: maxInterval, account_source: 'platform', account_ids: [], script_order_mode: 'random' }
...
async function createCustomerTask() {
  ...
  try {
    ...
    const response = await fetch('/api/customer/tasks', { method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, room_id: form.room_id.trim(), script_ids: orderedScriptIds }) })
  ...
  <div v-else class="customer-app" v-loading="customerLoading">
    ...
    <el-dialog v-model="createTaskVisible" title="创建直播任务" width="min(92vw, 620px)" align-center>
      <el-form label-position="top">
        ...
        <el-form-item label="直播间 ID"><el-input v-model="taskForm.room_id" placeholder="请输入直播间 ID" /></el-form-item>
        <el-form-item>
          <template #label>

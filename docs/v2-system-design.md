...
每个客户的自有账号总额度由“平台默认额度 + 客户额外额度”组成。平台默认额度初始为 3，管理员可在平台配置中调整，也可在客户编辑窗口单独增加额外额度。

创建直播任务必须选择账号模式：

- 平台账号模式由系统从平台账号池随机分配配置数量的闲置账号。
- 自有账号模式由客户从自己已登录且可用的账号中选择一个或多个。

两种模式分别配置每任务价格。任务创建时将当前价格以分为单位写入 `billing_amount_cents`，作为后续订单和账单模块的价格快照；当前版本只记录费用，不执行余额扣款。

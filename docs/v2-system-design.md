...
抖音账号分为 `platform`（平台账号）和 `customer`（客户自有账号）。客户自有账号必须记录 `owner_customer_id`，所有客户侧查询、登录、删除与任务分配都按当前客户校验归属。

客户订阅标准版、高级版或至尊版套餐。套餐定义自有抖音账号基础额度、平台账号分配数量、活动任务上限和单任务话术上限；客户单独购买的账号额度在套餐基础额度上累加。

创建直播任务必须选择账号模式：

- 平台账号模式由系统根据客户当前套餐，从平台账号池随机分配对应数量的闲置账号。
- 自有账号模式由客户从自己已登录且可用的账号中选择一个或多个。

创建任务不单独收费。管理员维护各套餐价格和额外账号额度单价，客户在个人中心创建购买订单。订单经支付渠道确认成功后自动开通对应权益，不经过管理员审批。`purchase_orders` 保存支付订单及其状态。
